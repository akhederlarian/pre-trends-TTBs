...
-	datasets: All datasets used in the estimation and production of all results (figures and tables) of the manuscript and online appendix in stata format (.dta). See list and description in step 3 of replication process.
-	figures: All figures of the manuscript and online appendix in pdf format (generated by “5-plot figures.do”). The figure file names are coded as in the manuscript (Figure 1 is “fig1.pdf”) and the online appendix.
...
-	results: Contains the estimation results in stata format used to create all figures of the manuscript and online appendix. Note the dta file names are coded as follows: (1) heterogeneity and robustness of section 6: `china’ for china and non-china differences, `2003’ for pre- and post-2003 differences, `metals’ for base metal sector and others differences, `placebo’ for placebo robustness, `ihs for zero trade flow robustness, `mass’ for scope of the TTB investigation; (2) specification: `event’ for standard event study, `extrap`n’’ for extrapolated approach with `n’ length of pre-investigation period, `static’ for static specification; (2) `avg’ and `aff’ for average trade effect and trade effect of affirmatively rules cases, respectively; (3) sample used (`base’, `rob1’, `rob2’, `gt_base’, `world’); (4) frequency (`qtr’, `yr’); (5) `val’,`qty’, and `uv’ are dependent variables: import value, quantity, and unit value; (6) specification of fixed effects (`fe8’ is ig,ist,s’t FEs, `fe2’ is ig,ist FEs, `fe4’ is ig,i-hs4-t FEs, `fe7’ is ig,ist,gt FEs.
-	tables: Contains the estimation results in txt format (generated by “4-estimation.do”) used to create all tables of the manuscript and online appendix. Note the labelling of the txt files is as follows: (1) specification: `event’ for standard event study, `extrap`n’’ for extrapolated approach with `n’ length of pre-investigation period, `static’ for static specification; (2) `avg’ and `aff’ for average trade effect and trade effect of affirmatively rules cases; (3) `base’ is baseline sample; (4) `qtr’ is quarterly frequency; (5) `val’,`qty’, and `uv’ are dependent variables: import value, quantity, and unit value; (6) `fe8’ are baseline specification of fixed effects.
